...
const FieldCardComp: React.FC<Props> = ({ card, selected, glowing, onClick }) => {
    ...
    const barrierHp = getBarrierHp(card);
    ...
    const hasAtkBuff = card.statuses?.some(s => s.name === 'attack_buff' && (s as any).value > 0);
    ...
    return (
        <div onClick={onClick} style={{
            ...
            <div style={{ position: 'absolute', top: 1, right: 1, display: 'flex', gap: 1, flexWrap: 'wrap', maxWidth: 20 }}>
                ...
                {hasShield && <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#44aaff' }} title="피해감소" />}
