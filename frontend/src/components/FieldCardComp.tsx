...
interface Props {
    card: FieldCard;
    selected?: boolean;
    glowing?: boolean;   // 액션 페이즈에서 행동 가능한 카드
    onClick?: () => void;
}
...
const FieldCardComp: React.FC<Props> = ({ card, selected, glowing, onClick }) => {
    ...
    // 스킬 데미지를 표시용 공격력으로 사용 (base_attack이 0인 경우)
    const displayAtk = card.attack > 0 ? card.attack : card.extra?.display_attack ?? card.attack;
    ...
    return (
        <div onClick={onClick} style={{
            ...
            <div style={{ display: 'flex', gap: 3, fontSize: 8, fontWeight: 700 }}>
                <span style={{ color: '#ff4466' }}>⚔{displayAtk}</span>
