...
def mizuki_kasa(caster: FieldCard, target: FieldCard, game: GameState) -> dict:
    ...
    candidates = [
        ...
        and not ally.has_status("heal_block")
        and not ally.has_status("frozen_state")
        and not ally.has_status("phoenix_rebirth_pending")
    ]
    if not candidates:
        return {"success": True, "skill": "치유의 삿갓", "healed": []}

    total_heal = int(game.get_skill_damage(caster, "skill_2", apply_attack_buff=False) or 5)
    healed_map: dict[str, int] = {ally.uid: 0 for ally in candidates}

    remaining = max(0, total_heal)
    index = 0
    ...
    logs = [{"uid": uid, "healed": healed} for uid, healed in healed_map.items() if healed > 0]
